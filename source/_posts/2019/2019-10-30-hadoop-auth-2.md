...
在上一篇文章中，我们分析了`Kerberos`协议的设计和通信过程。可以了解到，`Kerberos`主要实现了不在网络传输密码的同时又能在本地进行高性能鉴权。由于`kerberos`的协议设计相对复杂，看到评论有人还有疑问，这里再举一个例子来分析一下`kerberos`的安全性。
